/**
 * MoneyHook — 纯自动点击+播报 (不跳转)
 * WeChat 8.0.76
 *
 * 前提: 红包自动抢已实现; 转账页面你已有代码打开
 * 本类: 页面打开后自动点收款 + 读金额播报
 */
...
public class MoneyHook {
        ...
        try {
            ...
            XposedHelpers.findAndHookMethod(c, "onResume", new XC_MethodHook() {
                @Override protected void afterHookedMethod(MethodHookParam p) {
                    ...
                    sH.postDelayed(() -> {
                        String amt = scan(a.getWindow().getDecorView());
                        if (amt != null) {
    ...
    static String scan(View r) {
        ...
        if (r instanceof ViewGroup)
            for (int i = 0; i < ((ViewGroup) r).getChildCount(); i++) {
                String s = scan(((ViewGroup) r).getChildAt(i));
                if (s != null) return s;
            }
        return null;
    }
    ...
    static void hookTF(ClassLoader cl) {
        try {
            ...
            XposedHelpers.findAndHookMethod(c, "onResume", new XC_MethodHook() {
                @Override protected void afterHookedMethod(MethodHookParam p) {
                    click((Activity) p.thisObject, 1200);
                }
            });
    ...
    static void click(Activity a, int ms) {
        sH.postDelayed(() -> {
            List<View> list = new ArrayList<>();
            scanBtns(a.getWindow().getDecorView(), list);
            XposedBridge.log("[M-TF] ms=" + ms + " btns=" + list.size());
            for (View v : list) {
                String t = txt(v);
                XposedBridge.log("[M-TF]   " + v.getClass().getSimpleName() + " [" + t + "] clk=" + v.isClickable());
            }
            for (View v : list) {
                String t = txt(v);
            ...
            if (ms < 4000) click(a, ms + 1500);
            else XposedBridge.log("[M-TF] give up");
        }, ms);
    }

    static void scanBtns(View r, List<View> o) {
        if (r == null) return;
        if (r.isClickable() && r.isEnabled() && r instanceof Button) o.add(r);
        if (r instanceof ViewGroup)
            for (int i = 0; i < ((ViewGroup) r).getChildCount(); i++)
                scanBtns(((ViewGroup) r).getChildAt(i), o);
    }

    static String txt(View v) {
        if (v instanceof Button) return ((Button) v).getText().toString();
        if (v instanceof TextView) return ((TextView) v).getText().toString();
        return null;
    }

    static double df(Object o, String n) {
        try { Field f = o.getClass().getDeclaredField(n); f.setAccessible(true); return f.getDouble(o); }
        catch (Throwable t) { return -1; }
    }
    static int di(Object o, String n) {
