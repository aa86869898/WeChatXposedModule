...
public class AutoJoinGroup {
    ...
    // ========== 入口: hook IEvent.e() + FileOutputStream ==========
    ...
    public static void onImageMsg(Object e9) {
        try {
            ...
            Log.e(TAG, "缩略图: " + thumbPath + " exists=" + thumbFile.exists()
            ...
            if (!m.find()) {
                Log.e(TAG, "XML解析失败, 无CDN参数, 兜底缩略图");
            ...
            Log.e(TAG, "CDN参数: aeskey=" + aeskey.substring(0, 8) + "..."
                + " mid=" + cdnMidUrl.substring(0, 20) + "...");

            // === 核心: com.tencent.mm.app.s0.a(r0) 触发 CDN 下载 ===
            try {
                ...
                XposedHelpers.setObjectField(r0, "d", 
                    XposedHelpers.callMethod(e9, "H0"));       // msgSvrId
                ...
                Class<?> s0Clz = cl.loadClass("com.tencent.mm.app.s0");
                XposedHelpers.callStaticMethod(s0Clz, "a", r0);
                Log.e(TAG, "CDN下载已触发 → 等待 FileOutputStream hook 截获");
            } catch (Throwable e) {
                Log.e(TAG, "CDN触发失败: " + e.getMessage() + ", 启动轮询");
            }
    ...
    private static void startPolling(String thumbPath, long msgId, ClassLoader cl) {
        ...
        new Thread(() -> {
            for (int i = 0; i < 15; i++) {
                ...
                if (i % 5 == 4) Log.e(TAG, "轮询中... " + ((i + 1) * 2) + "s");
            }
        }, "qr-poller").start();
    }
    ...
    private static void triggerQrRaw(long msgId, String imgPath, ClassLoader cl) {
        try {
            ...
            Log.e(TAG, "QR触发: msgId=" + msgId + " path=" + imgPath);
