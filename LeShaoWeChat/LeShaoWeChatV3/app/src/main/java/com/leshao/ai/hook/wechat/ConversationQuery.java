 ...
 * <p>
 * 会话列表经 {@link AiDataProvider} 的 {@code sessions} 通道转储，
 * 供模块进程的白名单页「导入微信会话」使用。
 */
public final class ConversationQuery {

    private static final String TAG = "LeshaoAI.ConvQuery";

    /** AiDataProvider authority（与 manifest 声明一致）。 */
    ...
    /** 把会话列表写入 AiDataProvider（白名单页导入数据源）。 */
    public static boolean dumpSessions(Context context) {
        ...
        try {
            ...
            values.put("json", root.toString());
            cr.insert(SESSIONS_URI, values);
            Log.i(TAG, "会话已转储: " + sessions.size() + " 个");
            return true;
        } catch (Throwable t) {
