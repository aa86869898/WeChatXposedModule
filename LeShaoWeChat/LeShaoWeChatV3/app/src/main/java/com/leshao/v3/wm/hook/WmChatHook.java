...
public class WmChatHook {
    ...
    public static void initOnAppStart(ClassLoader cl) {
        try {
            ...
            hookSendMsgMgrDebug();
            hookVideoSendDebug();
            LogWriter.log(TAG, "initOnAppStart OK v798 build=v421 2026-08-10");
        } catch (Throwable t) {
...
private static boolean sendVideoToUser(String toUser, String videoPath) {
        LogWriter.log(TAG, "v798 sendVideo ENTER: to=" + toUser + " path=" + videoPath);
        if (sCL == null) throw new RuntimeException("sCL null");
        ...
        LogWriter.log(TAG, "v798 sendVideo duration=" + duration + "s size=" + f.length());
        ...
        sH.post(() -> {
            try {
                ...
                LogWriter.log(TAG, "v798 sendVideo dstPath=" + dstPath);
                if (dstPath != null) {
                    ...
                    fis.close();
                    fos.close();
                    LogWriter.log(TAG, "v798 sendVideo copied to " + dstPath);
                    ...
                    long msgId = (Long) XposedHelpers.callStaticMethod(
                            XposedHelpers.findClass("e01.x9", sCL), "x", e9);
                    XposedHelpers.setObjectField(v2, "n", msgId);
                    LogWriter.log(TAG, "v798 sendVideo built v2 msgId=" + msgId + " path=" + dstPath);
                    Object w2 = XposedHelpers.callStaticMethod(
                            XposedHelpers.findClass("v21.o2", sCL), "qj");
                    boolean ret = (Boolean) XposedHelpers.callMethod(w2, "x", v2, true);
                    LogWriter.log(TAG, "v798 sendVideo w2.x ret=" + ret);
                }
            } catch (Throwable t) {
                LogWriter.log(TAG, "v798 sendVideo fail: " + t.getClass().getName() + ": " + t.getMessage());
                java.io.StringWriter sw = new java.io.StringWriter();
                t.printStackTrace(new java.io.PrintWriter(sw));
                LogWriter.log(TAG, "v798 sendVideo stack: " + sw.toString());
            }
        });
        return true;
    }
