...
public class WmChatHook {
    ...
    public static void initOnAppStart(ClassLoader cl) {
        try {
            ...
            hookSendMsgMgrDebug();
            hookVideoSendDebug();
            LogWriter.log(TAG, "initOnAppStart OK v810 d3q+VFS+CDN build=v428 2026-08-24");
        } catch (Throwable t) {
...
private static boolean sendVideoToUser(String toUser, String videoPath) {
        LogWriter.log(TAG, "v810 sendVideo ENTER: to=" + toUser + " path=" + videoPath);
        if (sCL == null) throw new RuntimeException("sCL null");
        ...
        LogWriter.log(TAG, "v810 sendVideo duration=" + duration + "s size=" + f.length());
        ...
        sH.post(() -> {
            try {
                ...
                try {
                    ...
                    LogWriter.log(TAG, "v810 c3.a newFilename=" + newFilename);
                } catch (Throwable tc) {
                    LogWriter.log(TAG, "v810 c3.a fail: " + tc.getMessage());
                    ...
                    LogWriter.log(TAG, "v810 fallback newFilename=" + newFilename);
                }
                if (newFilename == null) {
                ...
                LogWriter.log(TAG, "v810 vfsVideo=" + vfsVideoPath + " vfsThumb=" + vfsThumbPath);
                ...
                LogWriter.log(TAG, "v810 w6.d video copy ok");
                ...
                if (thumbFile != null && thumbFile.exists()) {
                    ...
                    LogWriter.log(TAG, "v810 w6.d thumb copy ok");
                }
                ...
                boolean ok = (Boolean) XposedHelpers.callStaticMethod(d3Class, "q",
                        ...
                        null, "", null, "", "",
                        false, -1L, null, "", "");
                LogWriter.log(TAG, "v810 d3.q ret=" + ok);
                if (!ok) {
                ...
                LogWriter.log(TAG, "v810 d3.h OK, v2=" + v2Obj.getClass().getSimpleName());
                try {
                    ...
                    LogWriter.log(TAG, "v810 CDN rj taskId=" + taskId);
                    if (taskId != null && !taskId.isEmpty()) {
                        XposedHelpers.callMethod(wService, "wj", e9Msg2, 2, taskId, null);
                        LogWriter.log(TAG, "v810 CDN wj registered ok");
                    }
                } catch (Throwable tcdn) {
                    LogWriter.log(TAG, "v810 CDN register fail: " + tcdn.getMessage());
                }
                sentResult[0] = true;
            } catch (Throwable t) {
                LogWriter.log(TAG, "v810 sendVideo fail: " + t.getClass().getName() + ": " + t.getMessage());
                java.io.StringWriter sw = new java.io.StringWriter();
                t.printStackTrace(new java.io.PrintWriter(sw));
                LogWriter.log(TAG, "v810 sendVideo stack: " + sw.toString());
                sentError[0] = t;
            } finally {
                latch.countDown();
            }
        ...
        try {
            ...
            if (!finished) {
                LogWriter.log(TAG, "v810 sendVideo timeout for " + finalToUser);
                throw new RuntimeException("video send timeout for " + finalToUser);
            }
            if (sentError[0] != null) {
                throw new RuntimeException(sentError[0]);
            }
            LogWriter.log(TAG, "v810 sendVideo done: to=" + finalToUser + " sent=" + sentResult[0]);
            return sentResult[0];
        } catch (InterruptedException e) {
            LogWriter.log(TAG, "v810 sendVideo interrupted for " + finalToUser);
