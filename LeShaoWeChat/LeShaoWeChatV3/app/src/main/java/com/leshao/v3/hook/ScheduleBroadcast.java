...
public class ScheduleBroadcast {
    ...
    private static void installDiagHooks() {
        ...
        // a21.b0 — SendMsgService (ViewModel层)
        try {
            Class<?> a21b0 = XposedHelpers.findClass("a21.b0", sClassLoader);
            XposedBridge.hookAllMethods(a21b0, "vj", new XC_MethodHook() {
                @Override protected void beforeHookedMethod(MethodHookParam param) {
                    log("TRACE a21.b0.vj(" + param.args.length + ")");
                    if (param.args.length > 0 && param.args[0] instanceof java.util.List) {
                        java.util.List list = (java.util.List) param.args[0];
                        log("  vj: 待发送消息数=" + list.size());
                        for (int i = 0; i < Math.min(list.size(), 3); i++) {
                            Object m = list.get(i);
                            try {
                                log("  [" + i + "] type=" + XposedHelpers.callMethod(m, "getType")
                                    + " talker=" + XposedHelpers.callMethod(m, "N0")
                                    + " content=" + XposedHelpers.callMethod(m, "j"));
                            } catch (Throwable ig) {}
                        }
                    }
                }
            ...
            // 钩住所有方法以发现 tj (looper)
            for (java.lang.reflect.Method m : a21b0.getDeclaredMethods()) {
                final String mName = m.getName();
                XposedBridge.hookMethod(m, new XC_MethodHook() {
                    @Override protected void beforeHookedMethod(MethodHookParam param) {
                        if (mName.equals("vj") || mName.equals("tj") || mName.equals("xj") || mName.equals("yj"))
                            log("TRACE a21.b0." + mName + "(" + param.args.length + ")");
                    }
                });
            }
            log("TRACE: a21.b0 OK methods=" + a21b0.getDeclaredMethods().length);
        } catch (Throwable t) { log("TRACE: a21.b0 fail: " + t.getMessage()); }

        // jm.y — SendMsgService (备选混淆名, 从栈: jm.y.tj:17)
        try {
        ...
        // sm0.h.b — CGI 发送包装器 (所有 CGI 唯一出口)
        try {
            Class<?> sm0h = XposedHelpers.findClass("sm0.h", sClassLoader);
            XposedBridge.hookAllMethods(sm0h, "b", new XC_MethodHook() {
                @Override protected void beforeHookedMethod(MethodHookParam param) {
                    log("TRACE sm0.h.b(" + param.args.length + ") p0="
                        + (param.args[0] == null ? "null" : param.args[0].getClass().getSimpleName()));
                }
                @Override protected void afterHookedMethod(MethodHookParam param) {
                    log("TRACE sm0.h.b → result=" + (param.getResult() == null ? "null" : param.getResult().getClass().getSimpleName()));
                    if (param.getThrowable() != null) {
                        log("  sm0.h.b THROW: " + param.getThrowable().getClass().getSimpleName() + ": " + param.getThrowable().getMessage());
                    }
                }
            });
            log("TRACE: sm0.h.b OK");
        } catch (Throwable t) { log("TRACE: sm0.h fail: " + t.getMessage()); }

        // wr5.g.L — CGI 分发器 (对接 Mars 网络层)
        try {
            Class<?> wr5g = XposedHelpers.findClass("wr5.g", sClassLoader);
            XposedBridge.hookAllMethods(wr5g, "L", new XC_MethodHook() {
                @Override protected void beforeHookedMethod(MethodHookParam param) {
                    log("TRACE wr5.g.L(" + param.args.length + ")");
                }
            });
            log("TRACE: wr5.g.L OK");
        } catch (Throwable t) { log("TRACE: wr5.g fail: " + t.getMessage()); }
        ...
        try {
            ...
            for (java.lang.reflect.Method m : fh5w.getDeclaredMethods()) {
                ...
                XposedBridge.hookMethod(m, new XC_MethodHook() {
                    @Override protected void beforeHookedMethod(MethodHookParam param) {
                        if (mName.length() <= 3)  // 短方法名: k, j, etc.
