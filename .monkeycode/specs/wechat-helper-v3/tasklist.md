...
- [x] 2. 实现日志和上下文管理基础设施
  ...
  - [x] 2.2 实现 `ContextManager` 全局上下文管理器
...
- [x] 3. 检查点 - 确保项目结构可通过 gradle assembleRelease 编译，无任何 WeKit 依赖引用

- [x] 4. 实现 WCDB 密钥捕获与数据库连接（P1最高优先级）
  - [x] 4.1 实现 `MainHook` IXposedHookLoadPackage 入口
  ...
  - [x] 4.2 实现 `DatabaseProvider` WCDB Hook 与密钥捕获
  ...
  - [x] 4.3 实现 `ContactRepository` 联系人数据查询
...
- [x] 6. 实现微信设置页跳转入口
  - [x] 6.1 实现 `SettingsEntryHook` 设置页按钮
...
- [x] 7. 实现独立 Activity 基础界面
  - [x] 7.1 实现 `SettingsActivity` 基础壳
  - [x] 7.2 实现 `ContactPickerFragment` 联系人选择器
